...
export const STATES = {
  delaware: { slug: 'delaware', code: 'DE', name: 'Delaware', agency: 'DNREC', agencyLong: 'Delaware Division of Fish & Wildlife (DNREC)', guide: 'https://dnrec.delaware.gov/fish-wildlife/fishing/' },
  // Planned: new-jersey (NJDEP), maryland (MD DNR), virginia (VMRC / DWR), new-york (NYSDEC), connecticut (CT DEEP)
};
...
export function sizeText(zone) {
  if (!zone.size?.length) return zone.sizeNote || 'No size limit';
  return zone.size.map(oneSize).join('; ');
}
export function bagText(zone) {
  return zone.bagNote || (zone.bag == null ? 'No daily limit' : `${zone.bag} per person per day`);
}
...
export function makePages(regs) {
  ...
  function speciesPage(slug, date = new Date()) {
    ...
    const stateZone = sp.zones.find((z) => z.zone === 'state') || sp.zones[0];
    ...
    const description = `${sp.name} regulations in ${st.name}: ${sizeText(stateZone)}; ${bagText(stateZone).toLowerCase()}; season ${seasonText(stateZone).toLowerCase()}. ${summary}`.slice(0, 300);
    ...
    const faq = [
      { q: `What is the size limit for ${sp.name} in ${st.name}?`, a: `${sizeText(stateZone)} in ${st.name} state waters.` },
      { q: `How many ${sp.name} can you keep per day in ${st.name}?`, a: bagText(stateZone) + '.' },
    ...
    const jsonld = [
      ...
      { '@context': 'https://schema.org', '@type': 'Article', headline: title, description, image: speciesImage(sp), dateModified: regs.scraped, author: { '@type': 'Organization', name: 'SlotGauge' }, publisher: { '@type': 'Organization', name: 'SlotGauge', logo: { '@type': 'ImageObject', url: SITE + '/icon-192.png' } }, mainEntityOfPage: canonical },
    ];
    const body = `
...
<div class="hero"><img src="${speciesImage(sp)}" alt="${esc(sp.name)}" loading="eager"></div>
...
${sp.zones.map((z) => `<tr><td><strong>${esc(ZONES[z.zone] || z.zone)}</strong></td><td>${z.closed ? 'Closed to harvest' : esc(seasonText(z))}</td><td>${z.closed ? '—' : esc(sizeText(z))}</td><td>${z.closed ? '—' : esc(bagText(z))}</td></tr>`).join('\n')}
</tbody></table>
${sp.notes.length ? `<h3>Notes</h3><ul class="notes">${sp.notes.map((n) => `<li>${esc(n)}</li>`).join('')}</ul>` : ''}
<h3>Exact ${st.agency} wording</h3>
<dl class="raw"><dt>Season</dt><dd>${esc(sp.raw.season)}</dd><dt>Size</dt><dd>${esc(sp.raw.size)}</dd><dt>Limit</dt><dd>${esc(sp.raw.limit)}</dd></dl>
<p style="font-size:14px;color:var(--ink-2)">Source: <a href="${esc(sp.url)}" rel="noopener" target="_blank" style="color:var(--accent)">${st.agencyLong} species page</a>, captured ${esc(regs.scraped)}.</p>
...
<h2>Frequently asked</h2>${faq.map((f) => `<h3>${esc(f.q)}</h3><p>${esc(f.a)}</p>`).join('')}`;
    return layout({ title, description, canonical, body, jsonld, ogImage: speciesImage(sp) });
  }
...
${list.map((s) => { const z = s.zones.find((x) => x.zone === 'state') || s.zones[0]; const closed = s.status === 'closed' || z.closed; return `<tr><td><a href="${urlOf(s)}">${esc(s.name)}</a>${s.status === 'invasive' ? ' <small>(invasive)</small>' : ''}</td><td>${closed ? 'Closed' : esc(seasonText(z))}</td><td>${closed ? '—' : esc(sizeText(z))}</td><td>${closed ? '—' : esc(bagText(z))}</td></tr>`; }).join('\n')}
</tbody></table>
...
<p style="color:var(--ink-2)">Size limits, seasons and daily limits for every species ${st.agency} regulates, rewritten in plain English and rechecked against the official pages nightly. Last captured ${esc(regs.scraped)}.</p>
<h2>Most searched</h2><div class="chips">${popular.map((s) => `<a href="${urlOf(s)}">${esc(s.name)}</a>`).join('')}</div>
${groups.map((h) => `<h2><a href="/${st.slug}/${h}/" style="color:inherit;text-decoration:none">${HAB[h]} →</a></h2><div class="grid">${species.filter((s) => s.habitat === h).sort((a, b) => a.name.localeCompare(b.name)).map((s) => `<a href="${urlOf(s)}">${esc(s.name)}<small>${esc(sizeText(s.zones.find((z) => z.zone === 'state') || s.zones[0]))}</small></a>`).join('')}</div>`).join('')}
<div class="app"><h2 style="margin-top:0">Is it a keeper?</h2><p>Say what you caught, snap a photo, or type it. SlotGauge checks today's rules and answers KEEP or RELEASE.</p><a class="cta" href="/">Open SlotGauge</a></div>`;
  ...
  return { speciesPage, hubPage, statePage, sitemap, slugOf, urlOf, bySlug, state: st };
}
